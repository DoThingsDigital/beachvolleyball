...
| [~] 0.7 | Hostinger-VPS: Standort DE prüfen, Ubuntu 24.04 härten (SSH-Key, ufw, fail2ban, unattended-upgrades), Coolify mit Staging + Prod, Postgres-Container, nächtlicher `pg_dump` auf externen S3, Restore-Test | 3 | NF6 | SSH-Key ✓, Hostinger-Firewall (22/80/443/8080) ✓, fail2ban ✓, Updates+Reboot 02.09. ✓, Postgres-Container ✓, täglicher pg_dump lokal (14 Tage) ✓, Restore-Test 02.09. bestanden ✓; statt Coolify bewusst schlankes Compose+nginx (deploy/README). Offen: externes S3-Backup |
...
| # | Ticket | h | Ref | DoD |
|---|---|---|---|---|
| [x] 6.1 | Report Auslastung (Tag/Woche/Zeitfenster/Platz) mit CSV | 5 | L1 | – |
